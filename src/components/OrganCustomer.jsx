...
import { Bell, Home, Briefcase, ShoppingCart, LayoutGrid, Folder, Users, BarChart, UserCog } from "lucide-react";
...
<nav className="flex flex-col gap-4 w-full">
  {sidebarItems.map((item, index) => (
    <button
      ...
      className="flex items-center gap-4 cursor-pointer hover:bg-gray-400 p-2 rounded-lg transition-all font-bold focus:outline-none"
      onClick={() => setIsSticky(true)}
      type="button" // Add type to avoid unexpected behavior
    >
      {item.icon}
...
<div className="bg-gray-300 p-4 flex justify-center shadow-md gap-2">
  {navItems.map((nav, index) => (
    ...
    type="button"
      key={index}
      className="px-6 py-2 bg-[rgb(0,0,122)] text-white rounded-lg font-bold text-center shadow-md 
                 hover:bg-blue-800 transition-all duration-300 ease-in-out 
                 active:scale-95 w-[180px]"
    >
      {nav.name} ({nav.count})
    </button>
  ))}
        ...
        <div className="p-4 overflow-x-auto bg-white">
          <table className="w-full bg-white shadow-md rounded-lg overflow-hidden">
            <thead className="bg-gray-300">
              <tr>
              {["#", "C-ID#", "Creation Date", "Skill", "level", "First Name","Last Name","National ID", "Phone Number", "Email","County", "Subcounty", "profileStatus"].map((header, index) => (
