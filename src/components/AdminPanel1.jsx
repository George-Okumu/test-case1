      ...
      <div className={`flex-1 flex flex-col transition-all duration-300 ml-${isExpanded || isSticky ? '56' : '20'}`}>
        ...
        <div className="bg-gray-200 text-black flex justify-between items-center p-4 shadow-md pr-12">
          ...
          <div className="flex items-center gap-6">
            {/* Notification Bell */}
            <div
  role="button" // ✅ Fixed typo ("buttton" → "button")
  tabIndex={0}
...
>
  ...
  {isHovered && (
    <div className="absolute top-8 left-0 bg-white shadow-lg rounded-md p-4 w-64 border z-10">
      <p className="font-bold">Notifications</p>
      <ul className="mt-2">
        <li className="border-b p-2">New Order Received</li>
        <li className="border-b p-2">New Job Posted</li>
        <li className="p-2">System Update Available</li>
      </ul>
    </div>
  )}
</div>
        ...
        <div className="p-4 overflow-x-auto bg-white">
          <table className="w-full bg-white shadow-md rounded-lg overflow-hidden">
            <thead className="bg-gray-300">
              <tr>
                {["#", "req#", "Creation Date", "Category", "Type", "Managed by", "County", "Subcounty", "Status"].map((header, index) => (
